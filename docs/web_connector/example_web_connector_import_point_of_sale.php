 ...
 */
define('QB_QUICKBOOKS_MAILTO', 'ejay0811@gmail.com');
	...
	if ($Doc = $Parser->parse($errnum, $errmsg)) {
		...
		foreach ($List->children() as $Invoice) {
			...
			foreach ($arr as $key => $value) {
				$arr[$key] = mysqli_real_escape_string(QuickBooks_Utilities::driverFactory(QB_QUICKBOOKS_DSN), $value);
			}

			// Store the invoices in MySQL
			mysqli_query(
				QuickBooks_Utilities::driverFactory(QB_QUICKBOOKS_DSN),
				"
				REPLACE INTO
					qb_example_invoice
				(
			...
			mysqli_query(QuickBooks_Utilities::driverFactory(QB_QUICKBOOKS_DSN), "DELETE FROM qb_example_invoice_lineitem WHERE TxnID = '" . mysqli_real_escape_string($arr['TxnID']) . "' ") or die(trigger_error(mysqli_error()));
			...
			foreach ($Invoice->children() as $Child) {
				if ($Child->name() == 'InvoiceLineRet') {
					...
					foreach ($lineitem as $key => $value) {
						$lineitem[$key] = mysqli_real_escape_string(QuickBooks_Utilities::driverFactory(QB_QUICKBOOKS_DSN), $value);
					}

					// Store the lineitems in MySQL
					mysqli_query(QuickBooks_Utilities::driverFactory(QB_QUICKBOOKS_DSN), "
						INSERT INTO
						...
						) ") or die(trigger_error(mysqli_error(QuickBooks_Utilities::driverFactory(QB_QUICKBOOKS_DSN))));
				}
			}
		}
	}
...
function _quickbooks_salesorder_import_response($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents)
{
	...
	if ($Doc = $Parser->parse($errnum, $errmsg)) {
		...
		foreach ($List->children() as $SalesOrder) {
			...
			// Remove any old line items
			mysqli_query(QuickBooks_Utilities::driverFactory(QB_QUICKBOOKS_DSN), "DELETE FROM qb_example_salesorder_lineitem WHERE TxnID = '" . mysqli_real_escape_string(QuickBooks_Utilities::driverFactory(QB_QUICKBOOKS_DSN), $arr['TxnID']) . "' ") or die(trigger_error(mysqli_error(QuickBooks_Utilities::driverFactory(QB_QUICKBOOKS_DSN))));
